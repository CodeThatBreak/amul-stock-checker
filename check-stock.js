...
async function checkStock() {
  ...
  try {
    ...
    if (!product) {
      throw new Error(
        "Product not found in API response — alias may have changed",
      );
    }
    ...
    console.log(
      `[${new Date().toISOString()}] ${product.name || PRODUCT_ALIAS}: ${inStock ? "IN STOCK" : "out of stock"}`,
    );

    if (inStock) {
      await notify(
        `${product.name || "Amul Whey Protein"} is IN STOCK`,
        PRODUCT_URL,
      );
    }
